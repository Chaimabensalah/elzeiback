	   ...
	   public MyServiceSalaries MyServiceSalaries;
	    ...
	    //Modifier un Salaries
		 ...
		 @ResponseBody
		    ...
		    public ResponseEntity<String> update(@PathVariable("id") int id, @RequestBody Salaries updatSalaries) {

			Salaries existingSalaries = MyServiceSalaries.findById(id);
		        if (existingSalaries == null) {

		            return new ResponseEntity<>("Salaries not found", HttpStatus.NOT_FOUND);
		        }
	        	...
	        	MyServiceSalaries.save(existingSalaries);
		        return new ResponseEntity<>("Salarie updated successfully", HttpStatus.OK);
		    }
		   @ResponseBody
